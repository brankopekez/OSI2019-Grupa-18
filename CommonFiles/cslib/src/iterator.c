 ...
 * @file iterator.c
 *
 * This file implements a polymorphic version of the iterator and the tools necessary to construct types that support
 * iteration.  This module implements both the iterator.h and itertype.h interfaces.
 */
...
#include "itertype.h"
 ...
 /**
  * @brief This constant is stored in the header block of collection types to catch the case when a client tries to call
...
#define ITERATOR_PASSWORD 2718281828UL
  ...
  /**
   * @brief This cell type can vary in size according as to what follows the cell itself.  Iterators are implemented as
   * linked lists chained through the first word in the structure. Because the size of the actual data element can vary,
   * the data field is not represented as part of the structure. Instead, each cell is allocated dynamically with enough
   * memory for both the link field and the data value, however large it turns out to be. The list is arranged like a
   * queue, with a tail pointer to its final element for efficiency. The addToIteratorList function first checks to see
   * whether the element belongs at the end.  By checking for this case explicitly, it is often possible to avoid
   * searching the list at all.
   */
...
/**
 * @brief This type represents the concrete type for an iterator.
 */
 ...
 *
 * Most of the work of the package occurs in this function, which inserts the element addressed by dst into its correct
 * position. If the list is unordered or if the element belongs at the end, the element is immediately inserted at the
 * tail.  If not, the implementation walks the list to find the correct position. The local pointers have the following
 * interpretations:
 *
